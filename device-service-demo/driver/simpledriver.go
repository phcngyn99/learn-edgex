...
func (s *SimpleDriver) HandleReadCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []sdkModels.CommandRequest) (res []*sdkModels.CommandValue, err error) {
	...
	if len(reqs) == 1 {
		...
		if reqs[0].DeviceResourceName == "temperature" {
			// out, err := exec.Command("cat", "/sys/class/thermal/thermal_zone11/temp").Output()
			// if err != nil {
			// 	return nil, err
			// }
			...
			cv, _ := sdkModels.NewCommandValue(reqs[0].DeviceResourceName, common.ValueTypeInt64, int64(randomNumber))
			res[0] = cv
